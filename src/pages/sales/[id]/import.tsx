import type {
  ...
  export default function ImportSale(
    ...
    const importVerified = api.csvPorts.verifySaleCSV.useQuery(
      parsedCsvData,
    ...
    if (rows.length > 0) {
      return (
        <>
          <Head>
            <title>Import Sales Reconciliation CSV</title>
          </Head>
          <div className="pt-6"></div>
          <div className="rounded-lg bg-white px-6 pt-6">
            <div className="mb-2 block text-lg font-bold text-gray-700">
              Import Sales Reconciliation CSV
            </div>
            <Box
              sx={{
                height: "auto",
      ...
      return (
        <>
          <Head>
            <title>Import Sales Reconciliation</title>
          </Head>
          <div className="pt-6">
  ...
  export const getStaticPaths: GetStaticPaths = async () => {
    const salesReconcilations = await prisma.salesReconciliation.findMany({
      select: {
        id: true,
      },
    });
  
    const paths = salesReconcilations.map((salesReconcilation) => ({
      params: { id: salesReconcilation.id },
    }));
