...
import type { CSVBuybackInput, CSVBuybackInputId } from "../../../schema/imports.schema";
import "react-toastify/dist/ReactToastify.css";

export default function ImportBuyback(
  props: InferGetStaticPropsType<typeof getStaticProps>
) {
  const { id } = props;
  const router = useRouter();
  const [parsedHeaders, setParsedHeaders] = useState<string[]>();
  const [parsedCsvData, setParsedCsvData] = useState<CSVBuybackInput[]>();
  ...
  const importVerified =
    api.csvPorts.verifyBuybackCSV.useQuery(
      parsedCsvData,
      { enabled: !!headersVerified && headersVerified.data?.verified });
  const mutatedImport =
    api.csvPorts.addBuybackImport.useMutation();
  ...
  const handleUpload = (event: React.MouseEvent<HTMLElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: function (results) {
        ...
        const parsedData: CSVBuybackInput[] = [];
        results.data.forEach(function (value) {
          parsedData.push(value as CSVBuybackInput);
        })
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLElement>) => {
    ...
    try {
      ...
      const parsedDataTyped: CSVBuybackInputId[] = parsedData;
...
export const getStaticPaths: GetStaticPaths = async () => {
  const buybackOrder = await prisma.buybackOrder.findMany({
    select: {
      id: true,
    },
  });

  const paths = buybackOrder.map((buybackOrder) => ({
