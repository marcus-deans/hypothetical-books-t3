    ...
    if (mutation.isSuccess && mutation.data.success) {
        ...
        return (
            <div className="inline-block p-6 space-y-4 md:space-y-6 sm:p-8 w-3/12">
                ...
                <h1 className="text-xl font-bold leading-tight tracking-tight text-red-700 md:text-1xl text-red">
                ...
                <form className="space-y-4 md:space-y-6" action="#">
                    ...
                    <div>
                        <label className="block mb-2 text-sm font-medium text-white">Is this User an Admin?</label>
                        <input type="checkbox" name="admin" id="admin" checked={userInfo.admin} onChange={({ target }) => setUserInfo({ ...userInfo, admin: target.checked })} />
                    </div>
                    <button onClick={handleSubmit} type="submit" className="bg-gray-900 w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-primary-600 hover:bg-primary-700 focus:ring-primary-800">Submit User</button>
                </form>
            </div>
        );
    }
